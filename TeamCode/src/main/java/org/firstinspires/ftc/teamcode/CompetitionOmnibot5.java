 ...
 */
@TeleOp(name = "CompOmnibot5", group = "Competition")
public class CompetitionOmnibot5 extends OpMode {
    //motor variables
    DcMotor frontLeft;
    DcMotor frontRight;
    DcMotor backLeft;
    ...
    boolean shoot = false, reset = false, motorReset = false, aPressed = true, manualRest = false, sweepResetCheck = true,
            singleControllerMode = false, singleControllerToggle = false, p2Active = false;
    //gyro thingies
    long segmentTime;
    GyroSensor gyro;
    ...
    public void loop() {
        ...
        if (gamepad1.dpad_up && !(p2Active && singleControllerMode))
            controlMode = 3;
        if (gamepad1.dpad_right && !(p2Active && singleControllerMode))
            controlMode = 0;
        if (gamepad1.dpad_down && !(p2Active && singleControllerMode))
            controlMode = 1;
        if (gamepad1.dpad_left && !(p2Active && singleControllerMode))
            controlMode = 2;

        if (!singleControllerToggle && gamepad1.start && gamepad1.back) {
            if (!singleControllerMode)
                singleControllerMode = true;
            else
                singleControllerMode = false;
            singleControllerToggle = true;
        } else if (singleControllerToggle && !gamepad1.start && !gamepad1.back) {
            singleControllerToggle = false;
        }
        if (singleControllerMode) {
        ...
        if ((gamepad2.dpad_down  || (singleControllerMode && gamepad1.dpad_down && p2Active)) && !manualRest) {
            sweeper.setPower(-1);
            sweep = -1;
        }
        if (((gamepad2.dpad_left || gamepad2.dpad_right) || (singleControllerMode && (gamepad1.dpad_left || gamepad1.dpad_right) && p2Active)) && !manualRest) {
            sweep = 0;
            sweepResetCheck = false;
        }
        if ((gamepad2.dpad_up || (singleControllerMode && gamepad1.dpad_up && p2Active)) && !manualRest) {
            sweeper.setPower(1);
            sweep = 1;
        }
        //if (!gamepad2.a)//stops program from looping more than once, on shot per one button press
        //    aPressed = true;
        if ((gamepad2.a || (singleControllerMode && gamepad1.a && p2Active)) && sweep == 0 && aPressed && !shoot && !manualRest) {
        ...
        if (singleControllerMode) {
            telemetry.addData("Warning", "Single Controller Mode has been activated");
        }
        ...
        //Manual fire
        if (gamepad2.left_bumper || (singleControllerMode && gamepad1.left_bumper && p2Active))
        {
            manualRest = true;
            if (gamepad2.right_bumper  || (singleControllerMode && gamepad1.right_bumper && p2Active)) {
                shooter.setPower(.5);
            } else {
                shooter.setPower(0);
            }

            if (gamepad2.dpad_up || (singleControllerMode && gamepad1.dpad_up && p2Active))
                sweeper.setPower(.15);
            else
                sweeper.setPower(0);

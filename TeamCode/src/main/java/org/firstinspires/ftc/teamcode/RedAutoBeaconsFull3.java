        ...
        switch (control)
        {
            ...
            case MOVE_TO_BEACON2: {
                ...
                if (segmentTime + 1000 > time)
                    ...
                    navigateBlind(180 + extraAngle, .35, heading);

                if ((isVisible && posx > 1170) || (line.alpha() > 20 && segmentTime + 800 < time) || (spareSonar.getUltrasonicLevel() <= 65 && segmentTime + 800 < time) || ((color.blue() >= 4 || color.red() >= 4) && segmentTime + 800 < time)) {
            ...
            case MOVE_TO_PARK: {
                if (navigateTime(300, .5, 2000, heading)) {
                    allStop();
